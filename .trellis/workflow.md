...
Project delivery gate: run `scripts/test/local-gate.sh` and complete the local browser checks before preparing `scripts/release/prepare-release.sh`; production deployment must reference the pushed RC manifest and exact commit. Treat SSH banner/handshake/reset/timeout as `deployment_transport_failed`, inspect markers, and resume from the verified checkpoint without invalidating local implementation or tests.
