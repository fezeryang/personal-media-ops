...
| Quality gate exhausts candidates on the current selected platform | Rotate once through the remaining selected platforms in the same bounded round, preserving the rejection audit and appending a `quality_gate_platform_rotated` trace; do not stop before checking the requested platform set |
| Quality gate exhausts all candidates across the selected platforms | Enter Summarizing/`partial_completion` with the quality-gate stop reason; do not raise a generic `all research query candidates were rejected` failure |
