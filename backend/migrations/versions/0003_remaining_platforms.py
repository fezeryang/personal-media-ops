"""Expand crawler task platform constraint to all search adapters."""
...
revision: str = "0003_remaining_platforms"
down_revision: str | Sequence[str] | None = "0002_multiplatform_tasks"
...
def _rebuild(platform_constraint: str) -> None:
    ...
    op.rename_table("crawler_tasks", "crawler_tasks_before_remaining_platforms")
    _create_tasks_table(platform_constraint)
    op.execute(
        f"""
        INSERT INTO crawler_tasks ({columns})
        SELECT {columns}
        FROM crawler_tasks_before_remaining_platforms
        """
    )
    op.drop_table("crawler_tasks_before_remaining_platforms")
    op.create_index(
